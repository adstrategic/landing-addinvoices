...
export default function InvoiceByVoicePage() {
  ...
  const parseVoiceCommand = (text: string) => {
    ...
    const amountMatch =
      text.match(/\$\s?([\d,.]+)/) || text.match(/([\d,.]+)\s?dollars/i);
  ...
  const startListening = () => {
    ...
    if (!SpeechRecognition) {
      alert(
        "Voice input not supported in this browser. Type your command instead.",
      );
      return;
    }
  ...
  const downloadPDF = () => {
    ...
    doc.text(`DATE: ${new Date().toLocaleDateString()}`, 195, 50, {
      align: "right",
    });
    ...
    doc.setFontSize(16);
    doc.setFont("helvetica", "bold");
    doc.text(`TOTAL: $${parsedData.amount.toLocaleString()}`, 195, 120, {
      align: "right",
    });
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
          ...
          <div className="lg:col-span-5 space-y-8">
              ...
              <div className="relative z-10 space-y-6">
                <div className="flex items-center justify-between mb-2">
                  <label className="text-xs font-bold text-gray-500 uppercase tracking-widest flex items-center gap-2">
                    <Mic
                      className={`w-3 h-3 ${isListening ? "text-red-500" : "text-blue-500"}`}
                    />
                    Try a command
                  </label>
                  {isListening && (
                    <div className="flex gap-1">
                      {[...Array(3)].map((_, i) => (
                        <div
                          key={i}
                          className="w-1 h-3 bg-blue-500 animate-pulse rounded-full"
                        />
                      ))}
                    </div>
                  )}
            ...
            <div className="space-y-4">
              <h3 className="text-xs font-bold text-gray-500 uppercase tracking-widest ml-1">
                Example commands:
              </h3>
                ...
                <div className="grid grid-cols-2 gap-10">
                  <div className="space-y-1">
                    ...
                    <div className="text-sm font-bold">
                      {parsedData.dueDays} Days from now
                    </div>
                  </div>
                </div>
              </div>
