...
import { ChevronRight, FileText, Users, Lock, Download, AlertCircle } from "lucide-react";
...
export default function ContractGeneratorPage() {
  ...
  const downloadPDF = () => {
    ...
    doc.setFontSize(18);
    doc.text(contractType.toUpperCase() + " AGREEMENT", 105, 20, { align: "center" });
    ...
    const clauses = [
      { t: "1. SERVICES", c: `Contractor agrees to provide ${projectName || "the services"} as described: ${scope || "Professional services as required"}.` },
      { t: "2. COMPENSATION", c: `Client agrees to pay Contractor the sum of ${amount || "$0.00"} for the services described above.` },
      { t: "3. TERM", c: `The project will commence on ${startDate} and continue until completion or termination.` },
      { t: "4. CONFIDENTIALITY", c: "Both parties agree to protect and keep confidential any proprietary information shared during the course of this project." },
      { t: "5. GOVERNING LAW", c: `This agreement shall be governed by the laws of the State of ${governingState}.` }
    ];
    ...
    <div className="min-h-screen w-full relative bg-ad-main font-sans overflow-x-hidden">
      ...
      <main className="relative z-10 pt-28 pb-48 md:pb-20 px-4 md:px-6 max-w-6xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          <h1 className="text-4xl md:text-5xl font-black text-white mb-4 tracking-tight">Free Contract Generator</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-12 max-w-4xl mx-auto">
           {[
             { id: "freelance", icon: FileText, label: "Freelance Agreement", desc: "For independent projects" },
             { id: "services", icon: Users, label: "Services Agreement", desc: "For ongoing retainer work" },
             { id: "nda", icon: Lock, label: "NDA", desc: "For confidential info" }
           ].map(type => (
             <button
              key={type.id}
             ...
             >
                <type.icon className={`w-6 h-6 mb-4 ${contractType === type.id ? "text-blue-400" : "text-gray-500"}`} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
           ...
           <div className="bg-[#111111] border border-white/10 rounded-3xl p-8 space-y-8">
              <div className="space-y-6">
                 <div>
                    <h4 className="text-[10px] font-bold text-blue-500 uppercase tracking-widest mb-4">Contracting Parties</h4>
                 ...
                 <div>
                    <h4 className="text-[10px] font-bold text-blue-500 uppercase tracking-widest mb-4">Project Details</h4>
                 ...
                 <div className="grid grid-cols-2 gap-4">
                    <div>
                       <h4 className="text-[10px] font-bold text-blue-500 uppercase tracking-widest mb-4">Payment</h4>
                       <input 
                    ...
                    <div>
                       <h4 className="text-[10px] font-bold text-blue-500 uppercase tracking-widest mb-4">Start Date</h4>
                       <input 
                        type="date"
                 ...
                 <div>
                    <h4 className="text-[10px] font-bold text-blue-500 uppercase tracking-widest mb-4">Governing Law</h4>
                    <select 
                      value={governingState} onChange={e => setGoverningState(e.target.value)}
                    ...
                    >
                       <option value="New York" className="bg-[#111111]">New York</option>
                       <option value="California" className="bg-[#111111]">California</option>
                       <option value="Texas" className="bg-[#111111]">Texas</option>
                       <option value="Florida" className="bg-[#111111]">Florida</option>
                    </select>
                 </div>
              </div>
           ...
           <div className="bg-white rounded-lg shadow-2xl relative overflow-hidden aspect-[1/1.414] p-12 text-gray-800 text-[10px] leading-relaxed">
              ...
              <div className="mb-6">
                 This Agreement is made as of {new Date().toLocaleDateString()} by and between <strong>{yourName || "[Provider Name]"}</strong> ("Contractor") and <strong>{clientName || "[Client Name]"}</strong> ("Client").
              </div>

              <div className="space-y-6">
                 <div>
                    <div className="font-bold uppercase mb-1">1. Services</div>
                    <p>Contractor agrees to perform the following services for Client: {projectName || "[Project Name]"}. Scope: {scope || "[Details of work to be performed]"}.</p>
                 </div>
                 <div>
                    <div className="font-bold uppercase mb-1">2. Compensation</div>
                    <p>In consideration for the services, Client shall pay Contractor {amount || "[Amount]"}. Payment terms apply as per invoices issued.</p>
                 </div>
                 <div>
                    <div className="font-bold uppercase mb-1">3. Term</div>
                    <p>The term of this project shall begin on {startDate} and shall continue until the services are completed or this Agreement is terminated.</p>
                 </div>
                 <div>
                    <div className="font-bold uppercase mb-1">4. Confidentiality</div>
                    <p>Each party agrees that it will not disclose to any third party any confidential information of the other party without prior written consent.</p>
                 </div>
                 <div>
                    <div className="font-bold uppercase mb-1">5. Governing Law</div>
                    <p>This Agreement shall be construed and enforced in accordance with the laws of the State of {governingState}.</p>
                 </div>
              </div>
        ...
        <div className="mt-12 p-6 bg-amber-500/5 border border-amber-500/20 rounded-2xl flex items-start gap-4">
           ...
           <p className="text-[11px] text-amber-500/80 leading-relaxed italic">
             <strong>Legal Disclaimer:</strong> This contract template is provided for reference purposes only and does not constitute legal advice. AddInvoices is not a law firm. For legally binding agreements, we recommend consulting with a licensed attorney in your jurisdiction.
           </p>
        </div>
        ...
        <div className="mt-24 bg-[#111111] rounded-3xl p-10 md:p-16 border border-white/10 text-center relative overflow-hidden shadow-2xl">
          ...
          <p className="text-ad-secondary text-lg mb-10 max-w-2xl mx-auto leading-relaxed">
            With AddInvoices, your contracts and invoices live in the same place. Create a contract, collect a signature, and get paid.
          </p>
